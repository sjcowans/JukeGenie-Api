...
describe 'Tracks API' do
  describe 'GET /api/v1/tracks/:id' do
    it 'sends a Track within a key of data' do
      track = Track.create!(spotify_id: "chickenfired", name: "Chicken Fried")
      ...
      expect(parsed_track[:data][:attributes]).to have_key(:spotify_id)
      expect(parsed_track[:data][:attributes][:spotify_id]).to be_a(String)
    end
  ...
  describe "POST /api/v1/tracks" do
    it "is passed attributes required as JSON and creates a new track" do
      ...
      expect(created_track_formatted[:data][:attributes]).to have_key(:spotify_id)
      expect(created_track_formatted[:data][:attributes][:spotify_id]).to be_a(String)
    end
  ...
  describe "POST /api/v1/tracks/:id" do
    it "can patch attributes passed as JSON to a user" do
      ...
      expect(updated_track.spotify_id).to_not eq(og_track.spotify_id)
      expect(updated_track.name).to eq("Colder Wheather")
      expect(updated_track.spotify_id).to eq("ColderWheather")
    end
    ...
    it 'if there are missing attributes then an error is sent back' do
      ...
      expect(error_message).to eq({
            "errors": [
                {
                    "detail": "Validation failed: Spotify can't be blank"
                }
            ]
        }
          )
  ...
  describe 'delete /api/v1/tracks/{track_id}' do
    it 'can delete a track' do
      track1 = Track.create!(spotify_id: "chickenfired", name: "Chicken Fried")
      track2 = Track.create!(spotify_id: "ColderWheather", name: "Colder Wheather")
